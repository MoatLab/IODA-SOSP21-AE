			 ...
			 const struct blk_mq_queue_data *bd)
{
    ...
    if (req && req->bio) {
        ...
        req->bio->is_user_req = !test_bit(BIO_TIFA_RFW, &req->bio->bi_tifa_flags);
        ...
        struct nvme_completion *cqe)
{
    ...
    if (req->bio && req->bio->is_read) {
        printk("%s,rlat(us)=%llu,rfw=%d,PL=%d,cid=%d,gcrt=%llu,status=0x%x,%d\n",
               devname, ktime_us_delta(ktime_get(), req->bio->st), !req->bio->is_user_req, req->bio->pl, cqe->command_id, cqe->result.u64,
               le16_to_cpu(cqe->status) >> 1, (req->bio == req->biotail));
    } else if (!req->bio) {
