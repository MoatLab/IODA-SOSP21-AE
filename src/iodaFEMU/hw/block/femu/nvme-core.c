...
static void femu_flip_cmd(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    case FEMU_ENABLE_GC_DELAY:
        ...
        printf("Coperd,%s,FEMU GC Delay Emulation [Enabled]!\n", n->devname);
        break;
    case FEMU_DISABLE_GC_DELAY:
        n->ssd.sp.enable_gc_delay = false;
        printf("Coperd,%s,FEMU GC Delay Emulation [Disabled]!\n", n->devname);
        break;

    case FEMU_ENABLE_DELAY_EMU:
        n->ssd.sp.pg_rd_lat = 40000;
        n->ssd.sp.pg_wr_lat = 200000;
        n->ssd.sp.blk_er_lat = 2000000;
        n->ssd.sp.ch_xfer_lat = 0;
        printf("Coperd,%s,FEMU Delay Emulation [Enabled]!\n", n->devname);
        break;
    case FEMU_DISABLE_DELAY_EMU:
        ...
        n->ssd.sp.ch_xfer_lat = 0;
        printf("Coperd,%s,FEMU Delay Emulation [Disabled]!\n", n->devname);
        break;

    case FEMU_RESET_ACCT:
        n->nr_tt_ios = 0;
        n->nr_tt_late_ios = 0;
        printf("Coperd,%s,FEMU,Reset tt_late_ios/tt_ios,%lu/%lu\n", n->devname,
                n->nr_tt_late_ios, n->nr_tt_ios);
        break;

    case FEMU_ENABLE_LOG:
        n->print_log = true;
        printf("Coperd,%s,FEMU,Log print [Enabled]!\n", n->devname);
        break;

    case FEMU_DISABLE_LOG:
        n->print_log = false;
        printf("Coperd,%s,FEMU,Log print [Disabled]!\n", n->devname);
        break;

    case FEMU_SYNC_GC:
        n->ssd.sp.enable_gc_sync = true;
        printf("Coperd,%s,FEMU GC Synchronization [Enabled]!\n", n->devname);
        break;
    case FEMU_UNSYNC_GC:
        n->ssd.sp.enable_gc_sync = false;
        printf("Coperd,%s,FEMU GC Synchronization [Disabled]!\n", n->devname);
        break;

    case FEMU_ENABLE_LOG_FREE_BLOCKS:
        n->ssd.sp.enable_free_blocks_log = true;
        printf("Coperd,%s,FEMU log free blocks [Enabled]!\n", n->devname);
        break;
    case FEMU_DISABLE_LOG_FREE_BLOCKS:
        n->ssd.sp.enable_free_blocks_log = false;
        printf("Coperd,%s,FEMU log free blocks [Disabled]!\n", n->devname);
        break;
    case FEMU_WINDOW_1S:
        printf("FEMU_WINDOW_1S\n");
        n->ssd.sp.gc_sync_window = 1000;
        n->ssd.sp.gc_sync_buffer = 50;
        break;
    case FEMU_WINDOW_100MS:
        printf("FEMU_WINDOW_100MS\n");
        n->ssd.sp.gc_sync_window = 100;
        n->ssd.sp.gc_sync_buffer = 50;
        break;
    case FEMU_WINDOW_2S:
        printf("FEMU_WINDOW_2S\n");
        n->ssd.sp.gc_sync_window = 2000;
        n->ssd.sp.gc_sync_buffer = 50;
        break;
    case FEMU_WINDOW_400MS:
        printf("FEMU_WINDOW_400MS\n");
        n->ssd.sp.gc_sync_window = 400;
        n->ssd.sp.gc_sync_buffer = 50;
        break;
    case FEMU_WINDOW_200MS:
        printf("FEMU_WINDOW_200MS\n");
        n->ssd.sp.gc_sync_window = 200;
        n->ssd.sp.gc_sync_buffer = 50;
        break;
    case FEMU_WINDOW_40MS:
        printf("FEMU_WINDOW_40MS\n");
        n->ssd.sp.gc_sync_window = 40;
        n->ssd.sp.gc_sync_buffer = 0;
        break;
    case FEMU_WINDOW_10MS:
        printf("FEMU_WINDOW_10MS\n");
        n->ssd.sp.gc_sync_window = 10;
        n->ssd.sp.gc_sync_buffer = 0;
        break;

    case FEMU_ENABLE_DYNAMIC_GC_SYNC:
        printf("FEMU_ENABLE_DYNAMIC_GC_SYNC\n");
        n->ssd.sp.dynamic_gc_sync = true;
        break;
    case FEMU_DISABLE_DYNAMIC_GC_SYNC:
        printf("FEMU_DISABLE_DYNAMIC_GC_SYNC\n");
        n->ssd.sp.dynamic_gc_sync = false;
        break;

    case FEMU_ENABLE_HARMONIA:
        printf("FEMU_ENABLE_HARMONIA\n");
        n->ssd.sp.harmonia = true;
        break;
    case FEMU_DISABLE_HARMONIA:
        printf("FEMU_DISABLE_HARMONIA\n");
